...
function globalConfigFile() {
  const candidates = ["mammouth.jsonc", "mammouth.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      // Mammouth-branded config files load last and take highest precedence.
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mammouth.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mammouth.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const name of ["opencode", "mammouth"]) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir.endsWith(".mammouth") || dir === Flag.OPENCODE_CONFIG_DIR) {
