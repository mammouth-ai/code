...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run mammouth with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running mammouth server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        ...
        async function loop(client: OpencodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
          ...
          for await (const event of events.stream) {
            ...
            if (
              ...
              UI.empty()
              UI.println(
                `> ${event.properties.info.agent} · ${modelLabel(event.properties.info.providerID, event.properties.info.modelID)}`,
              )
              UI.empty()
