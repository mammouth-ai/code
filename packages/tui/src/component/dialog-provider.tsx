  ...
  return [
    ...pipe(
      ...
      map((provider) => ({
        ...
        category:
          provider.id === "mammouth-ai" ? "Mammouth AI" : provider.id in PROVIDER_PRIORITY ? "Popular" : "Providers",
      })),
    ),
    {
      type: "custom",
  ...
  const description = () =>
    ({
      ...
      "mammouth-ai": (
        <box gap={1}>
          <text fg={theme.textMuted}>
            Mammouth AI provides a curated selection of great models at an affordable price with a single API key.
          </text>
          <text fg={theme.text}>
            Go to <span style={{ fg: theme.primary }}>https://mammouth.ai/app/account/api</span> to get a key
          </text>
        </box>
      ),
